...
const createModal = (title, description) => {
  ...
  const closeModaleBtn = createElem("button", {
...
const product__Description = document.querySelectorAll(".product__description");
const product__Btn = document.querySelectorAll(".product__btn");

for (let i = 0; i < product__Btn.length; i++) {
  product__Btn[i].addEventListener("click", () => {
    const title = product__Title[i].textContent;
    const description = product__Description[i].textContent;
